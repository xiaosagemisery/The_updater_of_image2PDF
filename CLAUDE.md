...
打包成 exe（需要额外的 `requirements-build.txt`，见下方"打包成 exe"一节）：
...
该项目未配置 lint 步骤；唯一的"构建"步骤是上面这个可选的 PyInstaller 打包。
...
- `web_gallery.py` —— Gallery 的持久化。索引存在 `webdata/gallery.json`（可用环境变量 `IMAGE2PDF_DATA_DIR` 改路径），封面缩略图存在 `webdata/covers/<id>.jpg`。`DATA_DIR` 的默认值由 `_default_data_dir()` 计算：源码运行时是仓库下的 `webdata/`（已加入 `.gitignore`）；`getattr(sys, "frozen", False)` 为真（PyInstaller 冻结后设置的标记）时改成 `sys.executable`（exe 自身路径，不是 `__file__`）所在目录下的 `webdata/`。这里绝不能用 `__file__`——onefile 冻结后 `__file__` 指向 `sys._MEIPASS` 这个每次运行临时解压、进程退出就删除的目录，用它的话 Gallery 会每次重启就清空、封面全丢，这也是打包时最容易踩的坑（详见下方"打包成 exe"一节）。`item_id(pdf_path)` 用 PDF **完整绝对路径**的 SHA1 前 16 位作为 id，保证两个同名但不同目录的书（如两个都叫 `same.pdf`）不会共用同一张封面。`make_cover(src_image, id_)` 用 `PIL` 生成缩略图：先做 `ImageOps.exif_transpose`（否则手机竖拍照片会缩成横的），再处理 RGBA/带透明度的 P 模式（先在白底上合成，直接 `.convert("RGB")` 会把透明区域变黑），最后用临时文件 + `os.replace` 原子写入。`upsert_result(result, batch_id=None)` 按 `id` 原地更新已存在的记录（保留 `created_at` 和排列位置，只刷新 `updated_at`/`page_count`/`cover_file` 等），不存在则追加到末尾——因为重复转换同一个目录覆盖的是磁盘上同一个 PDF 文件，两条记录共享一张封面会导致删除时产生孤儿文件。`/api/gallery` 返回的 `cover_url` 带 `?v=<updated_at>` 查询参数用于让浏览器在重新转换后刷新缩略图缓存。`delete_item(id)` 只删 JSON 记录和封面文件，**绝不删除 PDF 本身**。

### 打包成 exe —— `*.spec` / `requirements-build.txt` / `.github/workflows/build-exe.yml`

用 PyInstaller 把两个入口分别打包成独立的 Windows exe，onefile 模式。exe 只能在 Windows 上构建（不能交叉编译），所以本地打包也只能在 Windows 上跑，CI 用的是 `windows-latest` runner。

- `image2PDF-web.spec` —— 入口 `webapp.py`，`name='image2PDF-web'`，`datas` 里显式带上 `templates/`、`static/` 两个目录（对应 `webapp._resource_dir()` 在冻结后从 `sys._MEIPASS` 读取这两个目录的假设，这里不打进去就对不上）。
- `image2PDF-cli.spec` —— 入口 `Image2PDF.py`，`name='image2PDF-cli'`，没有 `datas`（CLI 版不需要 templates/static）。
- 两份 spec 都用 `collect_data_files('reportlab')` 显式收集 reportlab 自带的字体/配置数据文件——reportlab 不是纯代码包，冻结环境里不显式收集可能在运行时找不到这些数据文件而出错；`console=True`（两个 exe 都保留控制台窗口：网页版靠控制台看启动地址、靠 Ctrl+C 停服务，命令行版本来就是控制台交互程序）。
- `requirements-build.txt` 只有 `pyinstaller>=6.0` 一行依赖，单独拆出来不进 `requirements.txt`，避免普通用户 `pip install -r requirements.txt` 时白装一个用不到的几十 MB 打包工具；**这个文件里不能写中文注释**——踩过一次坑：pip 解析 `.txt`/`.in` 依赖文件时，没有 BOM 的情况下是按系统默认编码（`locale.getpreferredencoding()`）解码整个文件，在 GBK/cp936 locale（常见的简体中文 Windows 默认设置）下解码 UTF-8 编码的中文字符会直接抛 `UnicodeDecodeError`，`pip install` 还没开始装包就先炸了。这个限制只对 pip 自己解析的 `requirements*.txt` 生效，`.spec` 文件是被 Python 解释器当普通源码执行的（Python 3 源码文件默认按 UTF-8 解码，不受系统 locale 影响），所以 `.spec` 里的中文注释不受影响。
- `Image2PDF.py` 的 `__main__` 结尾有一段 `if getattr(sys, "frozen", False): input("按回车键退出...")`——双击 exe 运行的窗口在脚本执行完会立刻关闭，用户来不及看到转换结果或报错信息；这段代码只在冻结环境下执行，源码方式 `python Image2PDF.py` 运行时 `sys.frozen` 不存在，行为不变。
- `Image2PDF.py` 模块顶层（导入的第一件事，`useA85` 设置之前）有一段 `sys.stdout`/`sys.stderr` 的 `reconfigure(errors="replace")`。这是给 CI 冒烟测试炸出来的真实 bug 修的：GitHub Actions 的 `windows-latest` runner 默认控制台代码页是英文 locale 下的 `cp1252`，`input("请输入图片所在的文件夹路径: ")` 这类带中文的 `print`/`input` 调用在这种代码页下会直接抛 `UnicodeEncodeError` 让整个进程崩溃（第一次跑 CI 时 `image2PDF-cli.exe` 的冒烟测试就是这样失败的，`image2PDF-web.exe` 因为启动方式不同没有触发但同样存在风险）。`errors="replace"` 只是让编码不出的字符退化成替代符而不是崩溃，对能正常显示中文的控制台（用户机器常见的 GBK/936 代码页，本来就能编码这些字符）是无操作，不影响真实用户看到的输出。放在 `Image2PDF.py` 模块顶层而不是只在 `__main__` 里，是因为 `webapp.py` 也 `import Image2PDF as image2pdf`，这样两个入口（含 `webapp.main()` 里同样带中文的 `print`）都能一并覆盖，不需要在 `webapp.py` 里再单独处理一遍。本地复现过这个崩溃：`chcp 1252` 切到英文代码页后跑 `image2PDF-cli.exe`，加上这个修复前会崩、加上之后能正常生成 PDF（只是控制台上的中文文字会变成乱码，但不影响功能）。
- `.github/workflows/build-exe.yml` —— push 到 `master`（且改动了代码，`paths-ignore` 排除纯文档改动）或 push 一个 `v*` 开头的 tag 时触发（也支持 `workflow_dispatch` 手动触发，用于 `paths-ignore` 导致 tag push 没能自动触发时的兜底）。步骤：装依赖 → `pytest -q`（测试不过就不打包/不发布）→ 用两份 spec 打包 → 冒烟测试（真的启动 `image2PDF-web.exe` 轮询 HTTP 响应、真的跑一次 `image2PDF-cli.exe` 转换检查是否生成 PDF——这一步是为了在发布前抓到"本地能跑但 CI 环境冻结后跑不起来"的问题，比如 `datas` 漏打了某个目录）→ 上传 artifact → 发布。发布策略：push 到 `master` 会先删除、再重新创建一个固定 tag `latest` 的 Release（滚动发布，下载链接 `.../releases/latest/download/image2PDF-web.exe` 永远不变，指向最新构建）；push `v*` tag 会额外创建一个带版本号、永久保留的正式 Release，两者互不影响。需要仓库 Settings → Actions → General → Workflow permissions 设为 **Read and write permissions**，否则创建 Release 会 403（这一步只能由仓库所有者在 GitHub 网页上操作，不是代码能解决的）。

**这个滚动发布策略是故意的取舍，不是 bug**：只 push 普通 commit 到 `master` 时，Releases/Tags 页面**只会看到 `latest` 这一个**，看不到任何历史版本——因为每次都是"删掉旧的 `latest`（连同它的 git tag 一起）再建一个新的"，上一次的构建产物不会保留。这是为了让下载链接永远固定不变、用户不用每次都去找新链接；代价就是没有历史快照。想要一份**不会被覆盖、永久留档**的版本，必须显式 `git tag v1.2.0 && git push origin v1.2.0` 打一个版本号 tag，工作流才会额外发布一个不受滚动逻辑影响的正式 Release。项目第一个正式 tag 是 `v1.2.0`（对应 `README.md` 里写的版本号），此后 Releases 页面才第一次同时出现 `v1.2.0` 和 `latest` 两条记录；以后每次想固化一个版本，都要重复"打 tag、push tag"这一步，不会自动发生。

Web 层新增的测试在 `test/webapp_test.py`，覆盖：`convert_images2PDF_*` 返回值契约、`__converted` 被 monkeypatch 成返回 `None` 时结果列表仍然完整（回归护栏，对应上面 `__converted` 的硬约束）、`convert_images2PDF_auto` 分派规则、`web_fs.normalize_dir`/`parent_of` 的边界情况、`/api/browse` 权限错误返回 403、`web_gallery` 的封面生成（RGBA→RGB）与原地更新/删除逻辑、`__split_double_page_images` 按 `right_page_first` 正确决定左右半图顺序（用左右两色拼图 + 像素采样断言）、`double_page=False`（默认）时完全不触发拆分逻辑、拆分产生的临时目录在转换结束后被清理、`__build_arg_parser`/`__validate_cli_args` 的双页参数校验、`/api/convert` 在 `double_page=true` 却缺 `right_page_first` 时返回 400、`on_page_progress` 在 `one_dir`/`more_dirs`/双页模式下上报的 `(book_number, book_total, page_done, page_total)` 序列是否正确、`webapp._compute_percent` 这个纯函数在单本/多本/尚无页级数据几种输入下的取值、`webapp._run_conversion` 端到端跑一次真实转换后 job 字典里 `percent`/`page_done`/`page_total` 是否正确演进到 100、空目录转换后 `job["status"]` 变成 `"error"` 而不是抛异常、`/api/convert` 请求路径完全不调用 `web_fs.preview`（monkeypatch 成一调用就报错来断言）且能立刻返回 `202`、`split_as_jpeg=False`（默认）时拆分出来的临时文件还是 `.png`（回归护栏）、`split_as_jpeg=True` 时是 `.jpg` 且 `right_page_first` 的左右顺序依然正确（JPEG 有损，断言改成"某个颜色分量明显更高"而不是像素精确相等）、带透明通道的半页图在 `split_as_jpeg=True` 时会先合成到白底、`/api/convert` 请求体带 `split_as_jpeg` 时能正确存进 `_new_job` 并透传到实际转换、`convert_images2PDF_one_dir`（含 `save_name` 自定义文件名的情况）和 `convert_images2PDF_auto` 的 `one_dir` 分派生成的 PDF 都落在 `file_dir` 的上一级而不是内部、`reportlab.rl_config.useA85` 导入 `Image2PDF` 后确实是 `0`、JPEG 直通和 PNG/raw 两条路径生成的 PDF 字节流里都不再出现 `/ASCII85Decode`（JPEG 路径额外断言 `/DCTDecode` 仍然存在，确认直通优化没有被误伤）、`web_gallery._default_data_dir()`/`webapp._resource_dir()` 在 monkeypatch `sys.frozen`/`sys.executable`/`sys._MEIPASS` 模拟冻结与非冻结两种环境下各自返回正确路径（不需要真的打包成 exe 就能测这两个纯函数,是 PyInstaller 打包相关改动的回归护栏，详见上方"打包成 exe"一节）。
